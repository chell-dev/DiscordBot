...
class Uwuify: Command() {

    private val emojiChance = 50
    private val stutterChance = 20

    override val names = arrayOf("uwu", "uwuify")
    override val description = "uwuify text"

    override fun exec(input: String, event: MessageReceivedEvent) {
        if(input.trim().isEmpty()) {
            event.channel.sendMessage("P-Pwease give me text to uwuify ^-^").queue()
        } else {
            event.channel.sendMessage(uwuify(input)).queue()
        }
    }

    // https://github.com/chell-dev/uwu.kt
